...
public:
	...
	// const la site- ptr a nu l mai modifica/ promitem
	cofetarieOnline(const char* site, int nrColaboratori, string nume, int nrAngajati,float* salarii, bool vegan, int an, float adaos) : Cofetarie( nume, nrAngajati ,vegan, an, adaos) {
	...
	cofetarieOnline(const cofetarieOnline& co) :Cofetarie(co){
		...
		strcpy_s(this->site, strlen(co.site) + 1, site);
	...
	float getNrMediuAngajatiPerColaboratori() {// int/int->int, adaug explicit(float) la primul sau la al doilea
		return(float) this->getNrAngajati() / this ->nrColaboratori;///nr ang e in cls parinte
	}
};

ostream& operator<<(ostream &out, const cofetarieOnline &co){

	out << (Cofetarie)co << endl;
	out << "Nr de colaboratori:" << co.nrColaboratori << endl;
	out << "Site: " << co.site;
...
int main() {
	cofetarieOnline co1;// cons fara param
	cout << co1.getNume();// dc derivarea e protected nu am acces la metode
	Cofetarie c1;
	cout << c1.getNume();

	float* vector = new float[3] {100, 200, 300};
	cofetarieOnline co2("cofetarie.ro",4,"cofetraie", 3, vector, true,2020, 15);
	cofetarieOnline co3(co2);
	cofetarieOnline co4 = co3;
	co4 = co3;
	cout << co4;

	int nrColaboratori = (int)co2;
	cout << endl << "nr de colaboratori:" << nrColaboratori << endl;// afoseaza 3(nr sal), nu 4-> fac cast la int ccare ret nr colab

	cout << "Nr mediu de angajati ptr fiecare colaborator:" << co2.getNrMediuAngajatiPerColaboratori();
